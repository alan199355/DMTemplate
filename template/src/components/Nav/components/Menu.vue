...
export default {
  ...
  render () {
    ...
    let renderMenu = (menuList, floor, upperMenuList) => {
      ...
      return menu.map(item => {
        if (item.children_list) {
          ...
          return (
            <el-menu-item onClick={() => this.addActiveTabs(item, floor, upperMenuList)} {...{ attrs: { 'key': item.menu_id, 'index': item.route_url, 'class': (item.route_url === this.activeTab ? 'active' : '') + ' el-menu-item' } }}>
              <i {...{ attrs: { 'class': (item.menu_css === '' ? 'fa-bars' : item.menu_css) + ' fa' } }}></i>
    ...
    return (
      <el-menu {...{ attrs: { 'class': 'el-menu-vertical-demo', 'collapse': this.isLeftNavCollapse }, props: { 'router': false } }}>
        {
